...
Rifinitura ha una coda, un archivio, un dettaglio e un elenco anomalie propri:
non compare mai nelle lavorazioni ordinarie. Le impostazioni vengono mostrate
come fotografia non modificabile della partenza; contatori e punto di ripresa
derivano dallo stesso ledger persistito per riga.

### Collaudatore interno

Il collaudatore non è una pagina e non avvia run proprie. Sorveglia ogni run
reale, confronta opzioni persistite, risultati e riletture Cloud e conserva le
anomalie nel flusso a cui appartengono. `VIA PIETRO COLLETTA` resta il perimetro
operativo concordato quando si eseguono collaudi manuali controllati.
...
Le lettere dei civici non vengono eliminate: `195/C` viene trasferito come **Civico `195`** e **Lettera `C`**, sia in creazione sia sovrascrivendo una scheda esistente. Una scheda con la stessa terna catastale, la stessa via, lo stesso Comune e il solo suffisso mancante viene riconosciuta e corretta; `195A` e `195B` non sono invece considerate equivalenti.

Il desktop è centrato sul **Registro operativo**. **Acquisisci e conserva** crea subito un job Supabase e memorizza ogni riga SISTER completata, con stato, anomalia, nominativi e cursore esatto, ma non apre il CRM; **Acquisisci e importa** usa la stessa raccolta durevole e avvia Tecnocloud soltanto quando tutte le varianti sono concluse e i dati obbligatori superano la validazione. Posizione corrente e righe già concluse restano contatori distinti: una pausa sulla terza riga mostra `3/N` e indica separatamente le due righe già salvate. Il checkpoint viene aggiornato dopo ogni riga e conservato anche nel record, che può essere ripreso direttamente un altro giorno. Lavorazione, Rifinitura e Portoni mantengono registri e motori distinti. Una variante fallita non viene mai trattata come vuota.
