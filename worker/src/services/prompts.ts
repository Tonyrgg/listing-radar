...
export class WorkerPrompts implements PromptController {
  ...
  async confirmSave(summary: string): Promise<AssistedDecision> {
    stdout.write(`\n${summary}\n`);
    const answer = (await this.readline().question("[C]onferma  [S]alta  [M]odifica manualmente  [V]erificare: ")).trim().toLowerCase();
  ...
  async reviewAcquisition(review: AcquisitionReview): Promise<AcquisitionReviewDecision> {
    ...
    const answer = (await this.readline().question("[P]rosegui  [S]alva per dopo  [A]nnulla: ")).trim().toLowerCase();
  ...
  async confirmMerge(summary: string): Promise<MergeDecision> {
    stdout.write(`\n${summary}\n`);
    const answer = (await this.readline().question("[C]onferma merge  [M]anuale: ")).trim().toLowerCase();
    return answer.startsWith("m") ? "manual" : "confirm";
  }

  async waitForManualEdit(): Promise<void> {
    await this.readline().question("Completa la modifica nel browser, poi premi Invio per proseguire. ");
  }
