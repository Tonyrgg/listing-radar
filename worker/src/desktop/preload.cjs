...
contextBridge.exposeInMainWorld("propertyWorker", {
  ...
  resumeAcquisition: (jobId) => ipcRenderer.invoke("desktop:resume-acquisition", jobId),
  skipAcquisitionRecord: (values) => ipcRenderer.invoke("desktop:skip-acquisition-record", values),
  ...
  savePortoni: (values) => ipcRenderer.invoke("desktop:save-portoni", values),
  generatePortoni: (values) => ipcRenderer.invoke("desktop:generate-portoni", values),
  ...
  mergeCompletedRuns: (values) => ipcRenderer.invoke("desktop:merge-completed-runs", values),
