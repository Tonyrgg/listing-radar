...
export function ContractMark({
  ...
  return (
    <span
      title={label}
      /* Due categorie, due icone: la tinta non identifica, la forma si. */
      className={`group/tooltip relative grid size-12 shrink-0 place-items-center rounded-[var(--lr-radius-control)] border border-[var(--lr-line)] bg-[var(--lr-raised)] text-[var(--lr-ink-2)] ${className ?? ""}`}
    >
      <Icon aria-hidden="true" className="size-5" />
      <span className="pointer-events-none absolute left-1/2 top-full z-20 mt-2 -translate-x-1/2 whitespace-nowrap rounded-[var(--lr-radius-control)] border border-[var(--lr-line)] bg-[var(--lr-raised)] px-2 py-1 text-[length:var(--lr-text-label)] font-semibold text-[var(--lr-ink)] opacity-0 transition-opacity group-hover/tooltip:opacity-100 group-focus-within/tooltip:opacity-100">
        {label}
      </span>
...
export function PropertyTypeMark({
  ...
  return (
    <span
      title={config.label}
      className={`group/tooltip relative inline-flex items-center gap-2 rounded-[var(--lr-radius-control)] border border-[var(--lr-line)] bg-[var(--lr-raised)] text-[var(--lr-ink-2)] ${
        compact ? "min-h-8 px-2 text-[length:var(--lr-text-meta)]" : "min-h-10 px-3 text-[length:var(--lr-text-body)]"
      }`}
    >
      <Icon aria-hidden="true" className="size-4 text-[var(--lr-ink-3)]" />
      {!compact ? config.label : <span className="sr-only">{config.label}</span>}
      {compact ? (
        <span className="pointer-events-none absolute left-1/2 top-full z-20 mt-2 -translate-x-1/2 whitespace-nowrap rounded-[var(--lr-radius-control)] border border-[var(--lr-line)] bg-[var(--lr-raised)] px-2 py-1 text-[length:var(--lr-text-label)] font-semibold text-[var(--lr-ink)] opacity-0 transition-opacity group-hover/tooltip:opacity-100">
          {config.label}
        </span>
      ) : null}
    </span>
...
export function FeatureMark({
  ...
  return (
    <span
      title={label}
      className="group/tooltip relative grid size-9 place-items-center rounded-[var(--lr-radius-control)] border border-[var(--lr-line)] bg-[var(--lr-raised)] text-[var(--lr-ink-2)]"
    >
      <Icon aria-hidden="true" className="size-4" />
      <span className="pointer-events-none absolute bottom-full left-1/2 z-20 mb-2 -translate-x-1/2 whitespace-nowrap rounded-[var(--lr-radius-control)] border border-[var(--lr-line)] bg-[var(--lr-raised)] px-2 py-1 text-[length:var(--lr-text-label)] font-semibold text-[var(--lr-ink)] opacity-0 transition-opacity group-hover/tooltip:opacity-100">
        {label}
      </span>
      <span className="sr-only">{label}</span>
    </span>
...
export function VisualFact({
  ...
  return (
    <div className="flex min-w-0 items-center gap-2.5">
      <span className="grid size-8 shrink-0 place-items-center rounded-[var(--lr-radius-control)] bg-[var(--lr-raised)] text-[var(--lr-ink-3)]">
        <Icon aria-hidden="true" className="size-4" />
      </span>
      <div className="min-w-0">
        <p className="text-[length:var(--lr-text-label)] font-bold uppercase tracking-[.09em] text-[var(--lr-ink-3)]">
          {label}
        </p>
        <p className={`mt-0.5 truncate ${prominent ? "text-[length:var(--lr-text-record)] font-bold text-[var(--lr-ink)]" : "text-[length:var(--lr-text-body)] font-semibold text-[var(--lr-ink-2)]"}`}>
          {value}
        </p>
      </div>
    </div>
