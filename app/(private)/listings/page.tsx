...
import {
  PropertyRow,
  signalsFromOpportunity,
  type PropertyRowSignals,
} from "@/components/property-row";
import {
  Campo,
  Card,
  Chip,
...
export default async function ImmobiliPage({
  ...
  for (const opportunita of vista.data.opportunita) {
    ...
    segnaliPerProprieta.set(
      opportunita.propertyId,
      signalsFromOpportunity(opportunita),
    );
  }
  ...
  const filtriAttivi =
    Boolean(cerca) || chi !== "tutti" || stato !== "attivi" || fermeDa > 0;
  ...
  return (
    <div className="space-y-4">
      ...
      <AutoSubmitFiltersForm className="flex flex-wrap items-center gap-2">
        <Ricerca
          label="Cerca fra le proprietà"
          defaultValue={param(query.q, "")}
          placeholder="via, zona, agenzia…"
        />

        <Campo label="Chi vende" labelHidden className="min-w-40">
        ...
        <Campo label="Stato" labelHidden className="min-w-40">
        ...
        <Campo label="In vendita da" labelHidden className="min-w-40">
      ...
      {filtrate.length > visibili.length ? (
        <Meta className="px-1">
          Ne vedi {visibili.length} di {filtrate.length}: restringi la ricerca
      ...
      <Meta className="flex flex-wrap items-center gap-x-4 gap-y-1 px-1">
        <span className="inline-flex items-center gap-1.5">
          <UserRound
            aria-hidden="true"
            className="size-3.5 text-[var(--lr-warn)]"
          />{" "}
          venduta da un privato
        </span>
        <span className="inline-flex items-center gap-1.5">
          <Building2 aria-hidden="true" className="size-3.5" /> tenuta da
          un&apos;agenzia
        </span>
        <span>
          Il testo tratteggiato è dedotto, non dichiarato dalla fonte.
        </span>
      </Meta>
    </div>
  );
