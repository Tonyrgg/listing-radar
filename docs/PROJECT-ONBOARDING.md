...
- Nel matching commerciale l'ascensore obbligatorio esclude, non penalizza: una richiesta che lo pretende non deve vedere immobili che ne sono privi. L'unica eccezione è il piano terra, dove l'ascensore non serve. Passa solo l'ascensore dichiarato presente: una scheda nasce con l'ascensore a «no», quindi un valore assente vale «no», e senza piano valorizzato entrano soltanto gli immobili con ascensore (`src/lib/matching/elevator.ts`).
  Il dato di partenza arriva da due parti, e per mesi non e' arrivato da nessuna delle due. La richiesta lo dichiara nel campo CRM «Ascensore»: l'import lo traduce in una preferenza `required` e non tocca mai una preferenza gia' decisa a mano (`worker/src/services/request-feature-requirements.ts`); per l'archivio importato prima di questa regola c'e' `npm --prefix worker run requests:reconcile-features`, che di suo non scrive e applica solo con `--apply`. L'immobile invece dal gestionale l'ascensore non lo porta affatto, quindi si rileva a mano in `/portfolio/ascensori`, dove ogni risposta ricalcola subito i suoi abbinamenti. Finche' nessuno risponde il valore resta assente e per il motore vale «no», ma a schermo «ascensore non rilevato» e «senza ascensore» restano due frasi diverse: la prima dice che c'e' una scheda da completare, la seconda che qualcuno e' andato a guardare. Le etichette di una richiesta dicono cosa chiede il cliente («ascensore indispensabile», «gradito», «da evitare») e quando non lo dichiara non si stampa nulla: una pastiglia in meno e' piu' onesta di una che afferma il falso.
- Un abbinamento sotto l'80% non è proponibile: non entra nelle liste operative e non copre la richiesta. La soglia è dichiarata in `MIN_PROPOSABLE_SCORE` (`src/lib/matching/repository.ts`) e ripetuta nella funzione `matching_request_coverage`; se cambia, vanno cambiate entrambe.
...
È un osservatore interno in sola lettura che accompagna ogni run ordinaria e
conserva soltanto anomalie fondate su evidenze già persistite. Non apre pagine,
non avvia prove autonome e non ripete salvataggi nel Cloud.

Durante l'acquisizione impedisce che una risposta SISTER contenente righe venga
presentata come successo con zero immobili acquisiti. Se «Sviluppa tutti i
proprietari» è attivo, verifica inoltre che almeno un intestatario letto sia
stato effettivamente aperto. Dopo ogni import confronta le opzioni salvate nel
job con checkpoint e risultati: proprietari attesi e sincronizzati, riferimenti
Cloud e, per `killer`, attività realmente conservata come `Eseguito` con modalità
e descrizione coerenti.

Le anomalie confluiscono nel registro persistente `worker-errors.json` con
origine «Sorveglianza automatica», job, immobile e codice del controllo. Lo
stesso controllo sullo stesso job non crea duplicati alle riprese. La Cronologia
è l'unica superficie che le mostra; una run regolare non aggiunge rumore.
